...
if (IS_WSL) {
  ...
  if (EXTRA_QWEN_DIRS.length) console.log('  \x1b[36m[WSL]\x1b[0m Extra Qwen dirs:', EXTRA_QWEN_DIRS.join(', '));
...
function exportSessionMarkdown(sessionId, project) {
  ...
  const session = loadSessions().find(s => s.id === sessionId || s.id.startsWith(sessionId));
  const detail = loadSessionDetail(sessionId, project);
  const projectLabel = (session && (session.project_short || session.project)) || project || '(none)';
  const toolLabel = session ? session.tool : found.format;
  const parts = [
    `# Session ${sessionId}`,
    '',
    `**Tool:** ${toolLabel}`,
...
function getActiveSessions() {
  ...
  try {
    const psOut = execSync(
      'ps aux 2>/dev/null | grep -E "claude|codex|qwen|opencode|kiro-cli|cursor-agent" | grep -v grep || true',
